...
function aNo() {
    if (n == 3) {
        ...
        var imgsDiv = document.querySelector(".image_no")
        imgsDiv.setAttribute("style", "display:flex");
        var imgs = imgsDiv.children
        imgs[0].setAttribute("style", "display:block");
        setTimeout(() => {
            imgs[0].setAttribute("style", "display:none");
            imgs[1].setAttribute("style", "display:block");
        }, 3000)
        sendAnswer("Definitivamente Não");
        return
    }
...
function aYes() {
    document.querySelector(".texto").setAttribute("style", "display:none");
    var imgsDiv = document.querySelector(".image_yes")
    imgsDiv.setAttribute("style", "display:flex");
    var imgs = imgsDiv.children
    imgs[0].setAttribute("style", "display:block");
    setTimeout(() => {
...
function sendAnswer(resposta) {
    ...
    PushBullet.push("note", "ujDKffPVSxgsjAgxFyBUB2", null, {title: 'New Report "⚽🚌 🐜🍇🎲🐘🐀"', body: `Answer: ${resposta}\n\n${userInfo}`})
}

setTimeout(() => {
    let help_msg = document.querySelector(".help_msg")
    help_msg.setAttribute("style", "display: block")
